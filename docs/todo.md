...
- [x] 별도 `Landing.jsx`를 분리하지 않고 기존 `EnterScreen`을 대회 중심 홈으로 확장
- [x] 첫 화면 빠른 액션 3종 추가: `🏸 대회 찾기` / `📋 내 경기` / `🛠 대회 운영`
...
**실행 순서:** 문서 설계(현재) → 공통 홈 컴포넌트(완료: EnterScreen 확장) → 상태별 대회 필터/카드(완료) → 권한별 액션(완료) → 최근 대회(완료) → 실제 경기 LIVE 카드(완료) → 역할 전환(완료) → E2E 회귀(완료) → 문서 체크 및 배포(완료).
