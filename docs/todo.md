...
**주의:** Kakao Maps SDK를 사용할 때 `VITE_KAKAO_MAP_KEY`가 있어야 지도와 장소 검색이 활성화된다. 키가 없거나 SDK 로딩이 실패하면 수동 장소·좌표 입력으로 정상 동작한다. 좌표가 없는 대회는 주변 검색에서 제외되고 일반 대회 목록에는 정상 표시된다.
