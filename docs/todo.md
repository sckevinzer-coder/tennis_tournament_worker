...
- [ ] `Landing.jsx` 신규 컴포넌트 또는 `ParticipantHome` 내 공통 홈 섹션 구현
- [ ] `App.jsx`: 참가자 기본값을 유지하면서 대회 중심 홈을 첫 화면으로 연결(전체 역할 선택 복귀 금지)
- [ ] `api/tournament.js`: 오늘/예정/진행 중 대회 목록 API 호출 또는 기존 `/tournaments` 응답을 필터링하는 헬퍼 추가
- [ ] 기존 `EnterScreen`·대회 검색·대회 상세 화면을 재사용하고, 중복 목록 UI를 통합
- [ ] 운영자/최고관리자 액션은 `isOrganizer`·`isSuperAdmin` 상태에 따라 조건부 표시
- [ ] 최근 대회 저장은 계정 기반(`/me/tournaments`)을 우선 사용하고, 비회원 공개 대회 목록은 localStorage에 최근 대회 ID만 보존하는 방식 검토
- [ ] 기존 하단 네비·참가자 상세·내 정보 탭을 삭제하지 않고 홈에서 해당 화면으로 이동하는 래퍼만 추가
...
- [ ] 비로그인 첫 화면에서 대회 검색·대회 카드 클릭·상세 진입 가능
- [ ] 오늘/진행 중/예정 대회가 실제 상태에 맞게 우선 노출
- [ ] 검색 결과 1개/여러 개/0개 상태의 CTA와 빈 상태 확인
- [ ] 참가자 모드에서 운영자 액션이 숨겨지고 참가자 액션만 노출되는지 확인
- [ ] 운영자/최고관리자 로그인 후 대회 운영 액션이 노출되는지 확인
- [ ] 최근 대회 카드가 실제 `/me/tournaments` 데이터와 일치하는지 확인
- [ ] 기존 참가 신청·내 정보·내 경기·대진표·대회 상세 흐름 회귀 없음
- [ ] `npm run build`, Worker `tsc --noEmit`, `browser_e2e.mjs` 전체 회귀 통과
...
**주의:** 지도는 외부 API 없이 GPS + 수동 좌표 입력 방식이다. 좌표가 없는 대회는 주변 검색에서 제외되고 일반 대회 목록에는 정상 표시된다.
