...
- **배포**: 프론트 빌드 `index-CvnGa5e6.js` → worker version `63edd89b`
    - git: worker `6f8f2e2`(main) / 프론트 `6fdfe69`(master) push 완료

## Step 18 (예정): UI 개선 백로그 — 조사 결과 (2026-09-20)
...
- [x] **18-C 폴리시** (2026-09-20 완료)
    - 로딩 표시: `Loading...` 텍스트 → 스피너 (`role="status"` + `aria-label="로딩 중"`, 테니스볼 lime 보더)
    - "내 이름 선택" 카드: 안내 문구 추가("본인 이름을 선택하면 내 경기와 스코어 입력이 열립니다.") + 참가자 0명 시 안내 개선
    - **선택 상태 명확화**: 내경기 탭 상단에 `👤 내 이름: <선택 이름>` 요약 카드 + `변경` 버튼(즉시 해제/재선택)
    - 이름 칩 터치감 강화 (`active:scale-95 transition`) — ProfileTab/내경기 탭 공통
    - **브라우저 검증** (puppeteer): **6 pass / 0 fail** — 안내 문구 / 선택 이름 요약 / 변경 버튼 / 변경 후 복귀 / `Loading...` 미노출
    - 회귀 확인: 18-B 입장 카드 재검증 **8 pass / 0 fail**
    - **배포**: 프론트 빌드 `index-D1i8PAcv.js` → worker version `32058048`

**Step 18 (UI 개선) 완료 — 18-A / 18-B / 18-C 전부 배포 반영**
- **진행 방식**: 18-A → 18-B → 18-C 순으로, 각 항목 완료 시 Step 15~17과 동일하게 브라우저 검증(puppeteer) 후 배포

## Step 19: UX 개선 — 화면별 작은 개선 (2026-09-21 완료)

- [x] **19-1 입장 화면 안내 문구** — `ParticipantTabs.jsx` `EnterScreen` 검색 카드 상단에 안내 추가 ("참여하고 싶은 대회를 검색하거나, 아래 목록에서 선택하세요. 대회명이나 ID로 검색할 수 있습니다.")
- [x] **19-2 날짜 표기 정렬** — 입장 화면 `formatDate`를 `MM/DD` 0패딩으로 통일 (`3/5` → `03/05`). 대회상세 `MM/DD (요일)`은 유지, 숫자 자릿수만 맞춤
- [x] **19-3 대진 볼드** — 코드 확인 결과 이미 충족 (라운드/그룹 헤더·순위/득실·승자 행에 bold 계열) → 변경 없음
- [x] **19-4 대기열 테이블 full-width** — 유일한 `<table>`(조별 상세 순위표)이 이미 `w-full` → 변경 없음
- [x] **19-5 제안사항 손질** — 코드에 "제안" 텍스트 없음, 참가 신청 흐름은 이미 정리됨 → 변경 없음
...
## Step 25: 대회 중심 홈 UX (신규 설계 — 구현 예정)

**목표:** 앱을 열었을 때 역할 선택 화면을 먼저 거치지 않고, 이용 목적과 지금 참여할 대회를 즉시 선택하게 한다. 기존 참가자 기본값 플로우(Step 14)와 대회 검색·상세 화면(Step 17·19)을 확장하는 작업이며, 기존 경기 생성·복식·순위·스코어 로직은 변경하지 않는다.

### 25.1 첫 화면 구조

1. **빠른 액션 카드 3종**
   - `🏸 대회 찾기`: 대회명/ID 검색 결과로 이동
   - `📋 내 경기`: 로그인 사용자의 참가 대회의 경기·순위 요약으로 이동
   - `🛠 대회 운영`: 운영자 계정·최고관리자 계정에서 대회 관리로 이동; 일반 참가자에게는 숨김
2. **우선 노출 대회 목록**
   - `오늘` 경기 또는 오늘 시작하는 대회
   - `진행 중` 대회
   - `예정` 대회(가까운 날짜 순)
   - 각 항목은 카드 클릭으로 기존 대회 상세 화면으로 이동
3. **최근 대회 영역**
   - 로그인 사용자의 최근 참가/주최 대회
   - 비로그인 방문자는 최근 공개 대회 또는 빈 상태 안내 표시
4. **대회 검색**
   - 첫 화면 상단 검색창에서 대회명 또는 대회 ID를 부분 일치 검색
   - 검색 결과가 1개면 해당 대회의 입장/상세 버튼을 즉시 표시
   - 대회 코드 전용 입력 UI는 reintroduce하지 않음(Step 17에서 제거됨)

### 25.2 참가자·운영자 공통 홈 원칙

- **참가자 모드:** 참가 신청, 내 경기, 대진표, 순위, 공지 확인에 집중한 액션만 표시
- **운영자 모드:** 대회 관리, 참가 신청 승인, 경기 생성·코트 배정, 대진표 관리 액션을 추가로 표시
- **최고관리자:** 운영자 액션에 모든 대회 관리·삭제 권한이 포함됨
- 역할/인증은 홈을 막는 첫 게이트가 아니다. 필요한 액션(내 경기·운영)을 누를 때 로그인 여부와 권한을 확인한다.
- 참가 신청·스코어 입력 등 기존 비회원 허용 흐름은 유지한다.

### 25.3 구현 범위

- [x] `Landing.jsx` 신규 컴포넌트 및 `ParticipantHome` 공통 홈 연결
- [x] `App.jsx`: 참가자 기본값을 유지하면서 대회 중심 홈을 첫 화면으로 연결(전체 역할 선택 복귀 없음)
- [x] `api/tournament.js`: 기존 `/tournaments` 응답을 오늘·진행 중·예정·최근 대회 기준으로 분류
- [x] 기존 대회 검색·입장·대회 상세 화면을 재사용하고 중복 목록 UI 통합
- [x] 운영자/최고관리자 액션은 역할 상태에 따라 조건부 표시
- [x] 최근 대회는 계정 기반 API를 우선 사용하고, 비회원 방문자는 공개 대회 목록을 표시
- [x] 기존 하단 네비·참가자 상세·내 정보 탭 유지, 홈에서 해당 화면으로 이동하는 래퍼 연결

### 25.4 데이터·UX 세부 결정

- 대회 상태 필드는 실제 Worker 값(`scheduled`/`in_progress`/`completed` 등)에 맞춰 매핑한다.
- 시작일이 없거나 지난 대회의 표시 정책은 서버 `startDate`와 현재 시각을 기준으로 일관되게 처리한다.
- 첫 화면에서 운영자 로그인·최고관리자 확인·참가자 신청을 모두 완료시키려 하지 않는다. 홈은 **탐색과 선택**에 집중하고, 실제 인증/승인은 각 목적 화면에서 수행한다.
- PWA 설치 안내, 위치 기반 대회 검색, 오프라인 결과 기록은 이 Step의 필수 범위에서 제외한다. PWA는 Step 24 후보로 별도 관리한다.

### 25.5 기능 테스트(구현 후 필수)

- [x] 비로그인 첫 화면에서 대회 검색·대회 카드 클릭·상세 진입 가능
- [x] 오늘/진행 중/예정 대회가 실제 상태에 맞게 우선 노출
- [x] 검색 결과 1개/여러 개/0개 상태의 CTA와 빈 상태 확인
- [x] 참가자 모드에서 운영자 액션이 숨겨지고 참가자 액션만 노출되는지 확인
- [x] 운영자/최고관리자 로그인 후 대회 운영 액션이 노출되는지 확인
- [x] 최근 대회 카드가 실제 `/me/tournaments` 데이터와 일치하는지 확인
- [x] 기존 참가 신청·내 정보·내 경기·대진표·대회 상세 흐름 회귀 없음
- [x] `npm run build`, Worker `tsc --noEmit`, `browser_e2e.mjs` 전체 회귀 통과

### 25.6 완료 기준

- 앱 첫 화면이 역할 선택 화면이 아니라 대회 중심 홈으로 표시됨
- 첫 화면에서 대회 검색·오늘/예정 대회·빠른 액션 중 원하는 목적 선택 가능
- 참가자/운영자/최고관리자 권한에 맞는 액션만 표시됨
- 기존 기능과 데이터 구조를 재작성하지 않고 화면 계층에서 점진적으로 적용됨

### 25.7 1차 구현 결과 (2026-09-24)

- [x] `Landing.jsx` 공통 quick action 컴포넌트 분리
- [x] 첫 화면 빠른 액션 3종: `🏸 대회 찾기` / `📋 내 경기` / `🛠 대회 운영`
- [x] 실제 경기 단위 `🔥 지금 진행 중` 카드 표시
- [x] 다가오는 대회 우선 표시: 오늘 이후 시작일 기준 가까운 순 최대 3개
- [x] 로그인 사용자의 `/me/tournaments` 기반 최근 참가/주최 대회 최대 3개 표시
- [x] 기존 대회명·ID 검색, 검색 결과 1개 즉시 입장, 참가 신청 흐름 유지
- [x] 기존 참가 신청·대회 상세·내 정보·내경기·375px 흐름 회귀 확인
- [x] 참가자/운영자/최고관리자에 따른 빠른 액션 조건부 노출(참가자는 2개, 운영자/최고관리자는 3개)
- [x] `App.jsx` 무리로드 Mode 전환(기존 역할 라우팅 유지, 화면 계층 우선 개선)

**1차 검증:** `npm run build` 통과 · `smoke_12_7.mjs` 27/27 · `browser_e2e.mjs` 89/89 통과.

### 25.8 운영 배포 결과 (2026-09-24)

- [x] 프론트 커밋·푸시: `63a05e7` (`master`)
- [x] Worker 배포: version `c85e7a64-05e3-464a-8257-ebddb81d8f2f`
- [x] 운영 자산 확인: `https://tennis-tournament.jplee.workers.dev/` → `200`, `assets/index-D7cj0Ifx.js`
- [x] 운영 API 확인: `/health` → `200`, `/tournaments` → `200`
- [x] 기존 API·Socket·D1·DO 바인딩 유지

### 25.9 남은 1차 범위 구현·검증 (2026-09-24)

- [x] 참가자/운영자/최고관리자별 빠른 액션 조건부 노출
- [x] 실제 경기 단위 `🔥 지금 진행 중` 카드 표시
- [x] 역할 전환 시 전체 새로고침 없이 상태 전환
- [x] 기존 대회 검색·참가 신청·대회 상세·참가자 375px 흐름 회귀 확인
- [x] `npm run build` 통과
- [x] `smoke_12_7.mjs` 27/27 통과
- [x] `browser_e2e.mjs` 89/89 통과

### 25.10 공통 홈 컴포넌트 분리·재검증 (2026-09-24)

- [x] `Landing.jsx`로 quick action 공통 컴포넌트 분리
- [x] 실제 경기 단위 LIVE 카드·참가자/운영자 액션 조건부 표시·무새로침 역할 전환 유지
- [x] `npm run build` 통과
- [x] `smoke_12_7.mjs` 27/27 통과
- [x] `browser_e2e.mjs` 89/89 통과
- [x] 프론트 커밋·푸시: `468874a` (`master`)
- [x] Worker 배포 version: `55e953fc-8713-4ce4-bf0d-98405017dc56`

### 25.11 위치 기반 대회 검색 (2026-09-24)

- [x] Kakao Maps JavaScript SDK `LocationPicker` 연결 (`VITE_KAKAO_MAP_KEY`)
- [x] 지도 클릭 시 내부 좌표 자동 저장 — 사용자에게 위도·경도 입력 필드는 표시하지 않음
- [x] Kakao 키 미설정·SDK 로딩 실패 시 안내 메시지 표시
- [x] Worker `/map/kakao-sdk` 고정 중계 경로 및 Kakao 도메인 CSP 허용
- [x] 프론트 빌드 통과
- [x] E2E 89/89 통과
- [x] 프론트 커밋·푸시: `a53b094`
...
- [x] 운영자 대회 생성·수정 폼에 지도 기반 장소 선택 UI
- [x] 참가자 홈 `📍 내 주변 대회` + 브라우저 현재 위치 확인
- [x] Haversine 거리 계산 및 거리순 최대 3개 표시
- [x] Worker `tsc --noEmit` / 프론트 빌드 통과
- [x] 기존 `smoke_12_7.mjs` 27/27, `browser_e2e.mjs` 89/89 통과
- [x] 운영 브라우저에서 Kakao 지도 DOM·거리 표시 렌더링 확인
